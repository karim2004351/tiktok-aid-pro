...
function AIContentRecommendationsPanel() {
  ...
  const { data: recommendations, isLoading, refetch, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch(`/api/content-recommendations?niche=${selectedNiche}&audience=${encodeURIComponent(selectedAudience)}&count=5`);
        if (!response.ok) {
        ...
        toast({
          ...
          description: "حدث خطأ أثناء جلب التوصيات الذكية",
          variant: "destructive",
        });
        throw error;
      }
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-xl font-semibold text-white mb-2">خطأ في الاتصال</h3>
        <p className="text-slate-400 mb-6">تعذر الاتصال بالخادم. يرجى المحاولة مرة أخرى.</p>
...
export default function AdminPanel() {
  ...
  const { data: stats, refetch, error: statsError } = useQuery({
    queryKey: ["/api/stats"],
    queryFn: async () => {
      try {
        ...
        toast({
          ...
          description: "تعذر جلب إحصائيات النظام",
          variant: "destructive",
        });
        throw error;
      }
